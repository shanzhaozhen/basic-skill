...
### 7.5 如何使用 redis 和 zookeeper 实现分布式锁？有什么区别优缺点，分别适用什么场景。
...
### 7.6 如果有人恶意创建非法连接，怎么解决。

可以使用filter过滤处理

### 7.7 分布式事务的原理，优缺点，如何使用分布式事务。
...
### 7.16 一个在线文档系统，文档可以被编辑，如何防止多人同时对同一份文档进行编辑更新。

* 点击编辑的时候，利用redis进行加锁setNX完了之后 expire 一下
* 也可以用版本号进行控制

### 7.17 线上系统突然变得异常缓慢，你如何查找问题。
...
### 7.19 Dubbo 的原理，数据怎么流转的，怎么实现集群，负载均衡，服务注册和发现。重试转发，快速失败的策略是怎样的。
...
### 7.22 编程中自己都怎么考虑一些设计原则的，比如开闭原则，以及在工作中的应用。
...
### 7.23 设计一个社交网站中的“私信”功能，要求高并发、可扩展等等。 画一下架构图。
...
### 7.25 应用服务器怎么监控性能，各种方式的区别。



### 7.26 如何设计一套高并发支付方案，架构如何设计。



### 7.27 如何实现负载均衡，有哪些算法可以实现。



### 7.28 Zookeeper 的用途，选举的原理是什么。



### 7.29 Mybatis 的底层实现原理。



### 7.30 请思考一个方案，设计一个可以控制缓存总体大小的自动适应的本地缓存。

### 7.31 请思考一个方案，实现分布式环境下的 countDownLatch。
...
### 7.33 如何看待缓存的使用（本地缓存，集中式缓存），简述本地缓存和集中式缓存和优缺点。本地缓存在并发使用时的注意事项。

### 7.34 描述一个服务从发布到被消费的详细过程。
